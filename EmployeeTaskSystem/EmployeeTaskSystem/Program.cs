    ...
    internal class Program
    {
            ...
            public void viewTasks()
            {
                ...
                try
                {
                    ...
                    if (sqlcon.State == ConnectionState.Open)
                    {
                        SqlCommand cmd = new SqlCommand(query, sqlcon);
                        SqlDataReader reader = cmd.ExecuteReader();
                        Console.WriteLine("Task_id\t Emp_id\t Deadline\t Task\t Remark");
                        while (reader.Read())
                        {
                            Console.WriteLine(reader[0] + "\t " + reader[3] + "\t " + reader[2] + "\t" + reader[1]+"\t"+reader[5]);
                        }
                    }
                }
                catch (Exception ex)
        ...
        public class SqlConn
        {
            ...
            public void createTaskListTable()
            {
                ...
                {
                    sqlcon.Open();
                    string query = "create table TaskList(task_id int primary key IDENTITY(1,1),task varchar(50),deadline DATE,emp_id int,rating int,remark varchar(70))";
                    SqlCommand cmd = new SqlCommand(query, sqlcon);
                    int i = cmd.ExecuteNonQuery();
                    Console.WriteLine("No of rows affected: " + i);
                }
